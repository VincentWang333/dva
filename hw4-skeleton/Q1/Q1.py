...
def author():
    return "xwang3141 "  # replace gburdell3 with your Georgia Tech username.


def gtid():
    return 903757529  # replace with your GT ID number
    ...
    def run_pagerank(self, node_weights,  damping_factor=0.85, iterations=10):
        ...
        for it in range(iterations):
            ...
            for source, target in self.read_edge_file(self.edge_file):
                # Implement your code here
                #############################################
                new_pr_values[target] += pr_values[source] / \
                    self.node_degree[source]

                #############################################
            pr_values = [damping_factor * new_pr_values[i] +
                         (1 - damping_factor) * node_weights[i] for i in range(len(new_pr_values))]
        print("Completed {0}/{1} iterations. {2} seconds elapsed.".format(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="sample command: python Q1.py -i 5 -d 0.85 simplified_pagerank network.tsv")
    parser.add_argument(
        "command", help="Sub-command to execute. Can be  simplified_pagerank or personalized_pagerank.")
    parser.add_argument(
        "filepath", help="path of the input graph file(network.tsv)")
    ...
    if args.command == "simplified_pagerank":
        ...
        result = pr.run_pagerank(
            node_weights=node_weights, iterations=args.iterations, damping_factor=args.damping_factor)
